...
def create_iterator(data, label, batchsize, shuffle=False):
    ...
    for i in range(30000):
        inds = np.random.choice([i for i in range(5994)], batchsize, replace=True)
    #print('inds')
        for j in range(len(inds)):
            digit_indices = np.array(np.where(label == inds[j])[0])
        #print('digit_indices:', digit_indices)
            if len(digit_indices) != 0:
                ids = np.random.choice(digit_indices, ims_per_id, replace=True)
...
train_data, train_labels = read_utt_file(path)
#print(len(train_data), len(train_labels))

datas, labels = create_iterator(train_data, train_labels, 16, False)
#print(datas)
labels = list(labels)
#print(labels)
'''
...
def create_iterator(path, batchsize, shuffle=False):
    ...
    for i in range(2000):
        inds = np.random.choice([i for i in range(5994)], batchsize, replace=False)
    #print('inds')
        for j in range(len(inds)):
            digit_indices = np.array(np.where(label == inds[j])[0])
        #print('digit_indices:', digit_indices)
