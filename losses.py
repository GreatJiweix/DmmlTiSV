...
class TripletLoss(nn.Module):
    ...
    def forward(self, inputs, targets):
        ...
        dist_ap, dist_an = [], []
        #for i in range(n):
            #dist_ap.append(dist[i][mask[i] == 1].sort()[0][int(K/2)])
            #dist_an.append(dist[i][mask[i] == 0].max())
        for i in range(n):
            dist_ap.append(dist[i][mask[i] == 1].min())
            dist_an.append(dist[i][mask[i] == 0].max())
        dist_an = torch.stack(dist_an)
        dist_ap = torch.stack(dist_ap)
        #margin = torch.abs((torch.max(dist_ap) - torch.max(dist_an)).data.cpu())
        #print(margin)
        ...
        y.fill_(1)
        y = Variable(y)
        loss_c = self.ranking_loss(dist_ap, dist_an, y)
