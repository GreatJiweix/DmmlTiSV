...
from torch._C import _remove_worker_pids, _error_if_any_worker_fails
...
def default_collate(batch):
    ...
    elif isinstance(batch[0], _six.int_classes):
        return torch.LongTensor(batch)
    elif isinstance(batch[0], float):
        return torch.DoubleTensor(batch)
    elif isinstance(batch[0], _six.string_classes):
        return batch
    elif isinstance(batch[0], _six.container_abcs.Mapping):
        return {key: default_collate([d[key] for d in batch]) for key in batch[0]}
    elif isinstance(batch[0], _six.container_abcs.Sequence):
...
def pin_memory_batch(batch):
    ...
    elif isinstance(batch, _six.string_classes):
        return batch
    elif isinstance(batch, _six.container_abcs.Mapping):
        return {k: pin_memory_batch(sample) for k, sample in batch.items()}
    elif isinstance(batch, _six.container_abcs.Sequence):
...
class DataLoader(object):
    ...
    def __init__(self, dataset, batch_size=1, shuffle=False, sampler=None, batch_sampler=None,
        ...
        if batch_sampler is None:
            ...
            batch_sampler = Sampler.BatchSampler(sampler, batch_size, drop_last)
